...
class ToMonetaExtensionsTest {
    ...
    fun int_toMoney_wholeUnits() {
        val m: Moneta = 5.toMoneta(code = "usd", decimals = 2)
    ...
    fun long_toMoney_wholeUnits() {
        val m = 123L.toMoneta(code = "usd", decimals = 2)
    ...
    fun short_and_byte_toMoney() {
        val s: Short = 2
        val b: Byte = 1
        assertEquals("2.00", s.toMoneta(code = "usd", decimals = 2).toDecimalString(2))
        assertEquals("1.00", b.toMoneta(code = "usd", decimals = 2).toDecimalString(2))
    }

    @Test
    fun double_toMoney_and_rounding_default() {
        // Double -> parsed from toString(), then scaled to currency.decimals (2)
        val d = 1.235.toMoneta(code = "usd", decimals = 2) // default HALF_UP rounding in factory
        // 1.235 -> scaled HALF_UP -> 1.24
        assertEquals("1.24", d.toDecimalString(2))

        // negative double
        val dn = (-0.5).toMoneta(code = "usd", decimals = 2)
    ...
    fun number_toMoney_dispatches_correctly() {
        ...
        val m1 = n1.toMoneta(code = "usd", decimals = 2)
        assertEquals("7.00", m1.toDecimalString(2))

        val n2: Number = 0.125                 // Double
        val m2 = n2.toMoneta(code = "btc", decimals = 8)
    ...
    fun explicit_rounding_parameter_respected() {
        ...
        val mDown = 1.239.toMoneta(code = "usd", decimals = 2, rounding = Rounding.DOWN)
        assertEquals("1.23", mDown.toDecimalString(2))

        val mUp = 1.231.toMoneta(code = "usd", decimals = 2, rounding = Rounding.UP)
        assertEquals(
            "1.24",
    ...
    fun zero_and_negative_integers() {
        val z = 0.toMoneta(code = "usd", decimals = 2)
        assertEquals("0.00", z.toDecimalString(2))

        val neg = (-3).toMoneta(code = "usd", decimals = 2)
        assertEquals("3.00", neg.toDecimalString(2))
    }

    @Test
    fun large_values_and_btc_precision() {
        val oneBtc = 1.toMoneta(code = "btc", decimals = 8)
        assertEquals("1.00000000", oneBtc.toDecimalString(8))

        val big = 9_999_999_999L.toMoneta(code = "usd", decimals = 2)
